...
Пусть этот год будет для тебя началом нового счастья!`;
...
document.addEventListener('DOMContentLoaded', () => {
    function createSparkle() {
        ...
        sparkle.style.animationDuration = (Math.random() * 10 + 12) + 's, ' + (Math.random() * 2 + 2) + 's';
...
function startFinalScene() {
    ...
    setTimeout(() => {
        ...
        // показываем финальную строку через 5.5 секунд
        setTimeout(() => {
            if (finalNote) {
